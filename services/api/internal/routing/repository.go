...
func (r *PgRepository) ListRules(ctx context.Context, merchantID *string) ([]RoutingRule, error) {
	...
	for rows.Next() {
		...
		var f1, f2 *string
		var merchID *string
		if err := rows.Scan(&rr.ID, &merchID, &rr.Name, &minStr, &maxStr, &rr.Currency, &pm, &rr.PrimaryConnector, &f1, &f2, &rr.Strategy, &rr.Enabled, &rr.Priority); err != nil { return nil, err }
		if minStr != nil {
...
func (r *PgRepository) ListHealthSamples(ctx context.Context, connectorID ConnectorID, since time.Time) ([]HealthSample, error) {
	...
	for rows.Next() {
		var hs HealthSample
		if err := rows.Scan(&hs.ID, &hs.ConnectorID, &hs.Environment, &hs.LatencyMS, &hs.Success, &hs.ErrorCode, &hs.SampledAt); err != nil { return nil, err }
