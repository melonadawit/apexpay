...
func (s *Service) ListOpen(ctx context.Context, limit int) ([]Case, error) {
	...
	for rows.Next() {
		var c Case
		if err := rows.Scan(&c.MerchantID, &c.IdempotencyKey, &c.TxRef, &c.RequestHash, &c.ConnectorState, &c.CreatedAt, &c.Status, &c.ReviewerID, &c.ReviewerNote); err != nil {
			return nil, err
		}
...
func (s *Service) Decide(ctx context.Context, merchantID, key, decision, reviewerID, note string) error {
	if decision != "confirmed_paid" && decision != "confirmed_not_paid" && decision != "requires_connector_investigation" {
		return fmt.Errorf("invalid reconciliation decision")
	}
	...
	if command.RowsAffected() != 1 {
		return fmt.Errorf("open reconciliation case not found")
	}
	if decision == "confirmed_not_paid" {
		_, err = tx.Exec(ctx, `UPDATE idempotency_keys SET state='retry_authorized',response_code=409,response_body=COALESCE(response_body,'{}'::jsonb)||jsonb_build_object('resolution','confirmed_not_paid') WHERE merchant_id=$1 AND key=$2 AND state='manual_review'`, merchantID, key)
		if err != nil {
			return err
		}
	}
