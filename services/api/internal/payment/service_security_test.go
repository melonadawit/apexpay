...
func (r *securityTestRepo) CreatePaymentTx(context.Context, *Payment, string, string) error { return nil }
...
func TestCanonicalRequestHashNormalizesOnlyIntendedInputs(t *testing.T) {
	...
	if canonicalRequestHash(a) != canonicalRequestHash(b) { t.Fatal("normalization should yield the same idempotency hash") }
	b.Amount = decimal.RequireFromString("100.01")
	if canonicalRequestHash(a) == canonicalRequestHash(b) { t.Fatal("a changed amount must change the idempotency hash") }
}
...
func TestVerify2FADemoOTPIsLocalOnlyAndPersists(t *testing.T) {
	...
	if err := local.Verify2FA(context.Background(), "merchant-1", "payment-1", "123456"); err != nil { t.Fatalf("local demo challenge should persist: %v", err) }
	if repo.verifiedMerchant != "merchant-1" || repo.verifiedPayment != "payment-1" { t.Fatal("2FA verification was not persisted against the supplied tenant/payment") }
	production := &Service{repo: repo, allowDemoOTP: false}
	if err := production.Verify2FA(context.Background(), "merchant-1", "payment-1", "123456"); err == nil { t.Fatal("demo OTP must not work outside local mode") }
}
