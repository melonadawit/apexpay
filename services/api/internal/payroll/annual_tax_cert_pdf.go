...
func GenerateAnnualTaxCertPDFGo(data AnnualTaxCertData) ([]byte, error) {
	...
	pdf.SetXY(15, 14)
	pdf.CellFormat(0, 5, fmt.Sprintf("Certificate No: %s • TIN: %s • Employee: %s (%s) • %s • Bank %s • Pension %s", data.CertificateNo, data.MerchantTIN, data.EmployeeName, data.EmployeeCode, data.EmployeeNameAM, data.BankMasked, data.PensionNo), "", 0, "", false, 0, "")
	pdf.SetXY(15, 20)
	pdf.SetFont("Helvetica", "I", 8)
	pdf.CellFormat(0, 4, fmt.Sprintf("Department %s • Cost Center %s • Period %s • Generated %s", data.Department, data.CostCenter, data.Period, data.GeneratedAt.Format(time.RFC3339)), "", 0, "", false, 0, "")
	...
	pdf.SetXY(17, y+2)
	pdf.CellFormat(0, 5, fmt.Sprintf("Employee: %s %s (%s) • TIN %s • Pension %s • Bank %s • Cost Center %s • Department %s", data.EmployeeName, data.EmployeeNameAM, data.EmployeeCode, data.EmployeeTIN, data.PensionNo, data.BankMasked, data.CostCenter, data.Department), "", 0, "", false, 0, "")
	pdf.SetFont("Helvetica", "", 9)
	pdf.SetXY(17, y+9)
	pdf.CellFormat(0, 4, fmt.Sprintf("Merchant: %s • TIN %s • Certificate No %s • Year %d • YTD Gross %s • Taxable %s • Tax %s • Net %s • Employer Pension %s • Total Employer Cost %s", data.MerchantName, data.MerchantTIN, data.CertificateNo, data.Year, data.YTDGross.StringFixed(2), data.YTDTaxable.StringFixed(2), data.YTDTax.StringFixed(2), data.YTDNet.StringFixed(2), data.YTDPensionEmplr.StringFixed(2), data.TotalEmployerCost.StringFixed(2)), "", 0, "", false, 0, "")
	...
	pdf.SetXY(15, y)
	taxInfo := fmt.Sprintf("Tax Calculation: Year-to-date Gross %s • Taxable %s • Income Tax %s • Net Pay %s • Employer Pension %s • Total Employer Cost %s",
		data.YTDGross.StringFixed(2), data.YTDTaxable.StringFixed(2), data.YTDTax.StringFixed(2), data.YTDNet.StringFixed(2), data.YTDPensionEmplr.StringFixed(2), data.TotalEmployerCost.StringFixed(2))
	...
	pdf.SetFont("Helvetica", "B", 10)
	pdf.SetXY(15, y)
	pdf.CellFormat(180, 7, fmt.Sprintf("Employer Total Cost %s = Year-to-date Gross %s + Employer Pension %s", data.TotalEmployerCost.StringFixed(2), data.YTDGross.StringFixed(2), data.YTDPensionEmplr.StringFixed(2)), "1", 0, "L", false, 0, "")
	y += 10
	...
	pdf.SetXY(50, y)
	pdf.MultiCell(130, 3, fmt.Sprintf("Verification: %s\nCertificate No: %s • Merchant TIN: %s • Employee TIN: %s • Pension No: %s • Bank: %s • Cost Center: %s • RunRef: %s • Period: %s • Year: %d • YTD Gross %s • Taxable %s • Tax %s • Net %s • Employer Cost %s • Generated at %s",
		data.QRVerificationURL, data.CertificateNo, data.MerchantTIN, data.EmployeeTIN, data.PensionNo, data.BankMasked, data.CostCenter, data.RunRef, data.Period, data.Year, data.YTDGross.StringFixed(2), data.YTDTaxable.StringFixed(2), data.YTDTax.StringFixed(2), data.YTDNet.StringFixed(2), data.TotalEmployerCost.StringFixed(2), time.Now().Format(time.RFC3339)), "", "", false)

	y += 35
	pdf.SetFont("Helvetica", "I", 7)
	pdf.SetTextColor(100, 100, 100)
	pdf.SetXY(15, y)
	footer := fmt.Sprintf("This is a computer-generated Annual Income Tax Certificate and does not require a signature. Certificate No: %s • Merchant TIN: %s • Employee TIN: %s • Pension: %s • Bank: %s • Cost Center: %s • Year: %d • YTD Gross %s • Taxable %s • Tax %s • Net %s • Employer Cost %s • Generated at %s",
