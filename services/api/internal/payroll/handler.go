...
func (h *Handler) GetPayslipPDF(w http.ResponseWriter, r *http.Request) {
	...
	// For demo generate mock PDF data — real would use gofpdf + QR barcode
...
func (h *Handler) GetPayslipsZip(w http.ResponseWriter, r *http.Request) {
	runID := chi.URLParam(r, "id")
	pkghttp.WriteJSON(w, r, 200, map[string]string{"zip_url": fmt.Sprintf("https://vault.apexpay.et/payroll/%s/payslips.zip", runID), "message": "download all ZIP"})
}
