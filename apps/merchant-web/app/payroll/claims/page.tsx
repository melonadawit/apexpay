    ...
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 to-primary-50/20 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex justify-between items-start">
          <div>
            <h1 className="text-3xl font-bold">Reimbursements & Claims • የወጪ ማካካሻ • Receipt Upload MinIO Approval Manager→Finance • Ethiopia Business Practice</h1>
            <p className="text-sm text-muted-foreground mt-2">Claim types expense/medical/travel/other amount description receipt_file_key MinIO presigned 15m TTL &lt;5MB pdf/jpg/png file_hash receipt_file_hash approved_by_manager approved_by_finance manager_approved_at finance_approved_at rejection_reason is_taxable is_pensionable status pending/approved/rejected/paid per Ethiopia business practice • Outstanding modern UI glassmorphic dropzone dashed pulse on drag</p>
          </div>
          <button className="rounded-xl bg-primary text-white h-10 px-6 text-xs">+ Create Claim • Expense/Medical/Travel/Other • MinIO Receipt Upload</button>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="p-6 lg:col-span-2">
            <h3 className="font-semibold">Claims • Receipt MinIO &lt;5MB pdf/jpg/png • File Key Hash Integrity • Approval Manager→Finance • Outstanding Pipeline Visual Stepper</h3>
            <div className="mt-4 rounded-xl border overflow-hidden">
              <div className="grid grid-cols-8 gap-2 bg-muted p-3 text-[11px] font-semibold"><span>Employee</span><span>Type</span><span>Amount</span><span>Description</span><span>Receipt • MinIO</span><span>Status</span><span>Approval Flow</span><span>Action</span></div>
              {mockClaims.map(c => (
                <div key={c.id} className="grid grid-cols-8 gap-2 p-3 border-t text-xs hover:bg-muted/50">
                  ...
                  <span className="text-[11px]"><span className="font-mono">{c.receipt}</span><span className="block">Hash {c.receipt_hash} • MinIO presigned 15m • &lt;5MB pdf/jpg/png • Encrypted SSE-S3 • 7y retention NBE • File key merchants/{"{id}"}/kyc/{"{type}"}_{"{id}"}.pdf</span></span>
                  ...
                  <span className="flex flex-col gap-1"><button className="text-primary text-[11px]">Approve • Manager→Finance</button><button className="text-red-500 text-[11px]">Reject • Reason</button><button className="text-muted-foreground text-[11px]">View Receipt • MinIO presigned 15m</button></span>
                </div>
              ))}
            </div>

            <div className="mt-6 rounded-xl bg-blue-500/10 border border-blue-500/20 p-4 text-[11px]">
              <p className="font-semibold">Approval Flow Manager→Finance • Outstanding Pipeline Visual Stepper • Maker-checker</p>
              <div className="mt-3 flex items-center gap-2">
                <div className="flex items-center gap-2"><span className="h-6 w-6 rounded-full bg-primary text-white flex items-center justify-center">1</span><span>Employee creates claim amount description receipt upload MinIO presigned POST 15m TTL file_key &lt;5MB pdf/jpg/png file_hash receipt_file_hash tax_exempt_limit is_taxable is_pensionable status pending</span></div>
                <div className="h-0.5 w-8 bg-neutral-200" />
                <div className="flex items-center gap-2"><span className="h-6 w-6 rounded-full bg-amber-500 text-white flex items-center justify-center">2</span><span>Manager approval • Reporting manager checks receipt verification • approved_by_manager manager_approved_at • Status approved_by_manager</span></div>
                <div className="h-0.5 w-8 bg-neutral-200" />
                <div className="flex items-center gap-2"><span className="h-6 w-6 rounded-full bg-green-600 text-white flex items-center justify-center">3</span><span>Finance approval • Finance manager checks taxability pensionability • approved_by_finance finance_approved_at • Status approved</span></div>
                <div className="h-0.5 w-8 bg-neutral-200" />
                <div className="flex items-center gap-2"><span className="h-6 w-6 rounded-full bg-green-600 text-white flex items-center justify-center">4</span><span>Paid via next payroll run • Reimbursement non-taxable added after tax • Payroll item other_allowances reimbursement non-taxable • Status paid • Ledger no extra entry? Actually reimbursement non-taxable added to net after tax • Outstanding</span></div>
              </div>
            </div>
          </Card>

          <Card className="p-6">
            <h3 className="font-semibold">Receipt Upload • MinIO Presigned POST 15m TTL &lt;5MB pdf/jpg/png • File Key Hash Integrity • Encrypted SSE-S3 • 7y Retention NBE • Outstanding Dropzone Dashed Pulse on Drag • Glare Detection</h3>
            ...
            >
              ...
              <p className="mt-3 font-medium text-sm">Drop receipt here • MinIO presigned POST 15m • &lt;5MB pdf/jpg/png • Hash integrity sha256 streaming O(n) • Encrypted SSE-S3 • 7y retention NBE</p>
              <p className="text-[11px] text-muted-foreground mt-1">File types whitelist pdf/jpg/png • Fayda &lt;2MB per NIDP • Size &lt;5MB • ClamAV stub VirusScanner clean • File hash integrity file_hash unique index per merchant • Encrypted SSE-S3 MinIO versioning • Retention 7y per NBE • No plain FIN logs grep test CI • PII redact zerolog field filter • FIN only last4 responses • Account masked ****1234</p>
              <button className="mt-4 rounded-xl bg-primary text-white h-10 px-6 text-xs">Select File • Dropzone dashed pulse on drag • Preview thumbs • Hash integrity • Progress donut 0-100% • Outstanding Modern • Mercury/Linear inspiration • Glassmorphic</button>
            </div>
