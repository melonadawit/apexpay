    ...
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 to-primary-50 p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} className="rounded-2xl bg-gradient-to-br from-primary to-primary-light p-6 text-white shadow-medium">
            <p className="text-white/70 text-sm">TPV Today • ዛሬ — Real Recharts TPV + merchant_tpv_daily materialized view refreshed hourly worker + SWR polling 5s</p><p className="text-3xl font-bold mt-2">ETB {tpv.toLocaleString()}</p>
            <p className="text-xs mt-2 bg-white/20 inline-block px-2 py-0.5 rounded-full">+12% vs yesterday • ትናንት • GET /v1/payments?merchant_id + /v1/admin/connectors/health Recharts AreaChart</p>
            <div className="mt-4">
              <TPVRecharts />
            </div>
          </motion.div>
          ...
          <div className="lg:col-span-2 space-y-4">
            <Card className="p-4"><h3 className="font-semibold">Recent Payments • የቅርብ ክፍያዎች</h3>
              <div className="mt-3 space-y-2">
                {[
                  ...
                  <div key={p.tx} className="flex items-center justify-between rounded-xl border p-3 hover:bg-neutral-50">
                    <div className="flex items-center gap-3"><div className="h-8 w-8 rounded-full bg-green-100 text-green-700 flex items-center justify-center text-xs">✓</div><div><p className="text-sm font-medium">{p.amt} • {p.method} {p.twoFA && "• 2FA"}</p><p className="text-xs text-muted-foreground">{p.tx} • {p.ago} ago • routed via best</p></div></div>
            ...
            <Card className="p-4"><h3 className="font-semibold">Quick Actions • ፈጣን እርምጃዎች</h3>
              <div className="mt-3 grid grid-cols-3 gap-2">
                <button className="rounded-xl border p-3 text-sm font-medium hover:bg-neutral-50">Create Link • ሊንክ</button>
                <button className="rounded-xl border p-3 text-sm font-medium hover:bg-neutral-50">Pay Vendor • አቅራቢ ክፍያ</button>
                <button className="rounded-xl border p-3 text-sm font-medium hover:bg-neutral-50">Run Payroll • ደሞዝ</button>
              </div>
            </Card>
          </div>
          ...
          <GlassCard className="p-4 space-y-3">
            ...
            <div className="rounded-xl bg-white border p-3 text-sm">
              ...
              <div className="mt-2 space-y-2">
                <div className="flex gap-2"><span className="h-6 w-6 rounded-full bg-primary text-white flex items-center justify-center text-xs">1</span><span className="text-xs">Tool get_tpv → ETB 125,430</span><span className="ml-auto text-green-600 text-xs">✓</span></div>
                <div className="flex gap-2"><span className="h-6 w-6 rounded-full bg-primary text-white flex items-center justify-center text-xs">2</span><span className="text-xs">Tool create_payment_link 100 ETB</span><span className="ml-auto text-green-600 text-xs">✓</span></div>
              </div>
