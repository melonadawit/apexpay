        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="rounded-2xl border bg-white p-6 space-y-4">
            ...
            <div className="rounded-xl border p-3 bg-neutral-50">
              ...
              <div className="mt-2 flex gap-2"><button onClick={()=> setShowSecret(!showSecret)} className="rounded-lg border px-3 py-1 text-xs">{showSecret ? "Hide" : "Reveal"} • sk_live shown once</button><button className="rounded-lg bg-primary text-white px-3 py-1 text-xs">Create New Key • test/live separate live only after KYC active</button></div>
            ...
            <div className="rounded-xl bg-blue-50 border border-blue-200 p-3 text-xs">
              ...
              <pre className="mt-2 bg-black text-white rounded-lg p-3 overflow-auto text-[11px]">{`npm install apexpay-js
...
`}</pre>
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border bg-white p-4">
              ...
              <button className="mt-3 w-full rounded-xl bg-primary text-white h-10 text-sm">Save Endpoint • SSRF block private ranges Allowlist 10.0.0.0/8 172... 192.168 127.0.0.1</button>
              <div className="mt-3 space-y-1 text-xs">
                <div className="flex justify-between border-b py-1"><span>payment.succeeded → https://.../webhook</span><span className="px-2 py-0.5 rounded-full bg-green-100">success 200 attempt1 HMAC valid</span></div>
                <div className="flex justify-between border-b py-1"><span>refund.succeeded</span><span className="px-2 py-0.5 rounded-full bg-amber-100">failed retry backoff 2s 4s 8s</span></div>
              </div>
            </div>
